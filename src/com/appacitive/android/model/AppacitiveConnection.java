...
public class AppacitiveConnection {
	...
	public void createConnection(final AppacitiveObject object1,
			final AppacitiveObject object2, final AppacitiveResponseCallback callback) {
				...
				public Void run() throws AppacitiveException {
					...
					try {
						...
						if (object1 != null && object2 != null) {
							requestParams = AppacitiveConnection.this.createRequestParams(object1, object2);
						...
						if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
							...
							if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
								...
								try {
									...
									appacitiveError = AppacitiveHelperMethods.checkForErrorInStatus(statusObject);
									if (appacitiveError == null) {
										readConnection(responseJsonObject.getJSONObject("connection"));
									}
	...
	protected JSONObject createRequestParams(AppacitiveObject object1, AppacitiveObject object2) {
...
//				endPointB.put("articleid", this.mArticleBId+"");
	...
	public void searchConnectionWithRelationTypeAndQueryString(String relationType, final String query, final AppacitiveResponseCallback callback) {
		final Appacitive appacitive = Appacitive.getInstance();
		if (appacitive != null) {
			BackgroundTask<Void> searchTask = new BackgroundTask<Void>(null) {
				...
				public Void run() throws AppacitiveException {
					...
					try {
						if (query == null) {
							url = new URL(Constants.CONNECTION_URL + AppacitiveConnection.this.mRelationType + "/find/all");
						} else if (query != null) {
							//append to request params
							url = new URL(Constants.CONNECTION_URL + AppacitiveConnection.this.mRelationType + "/find/all" + "&%@" + query);
						}
						...
						if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
							...
							if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
								...
								while (jsonReader.hasNext()) {
									...
									name = jsonReader.nextName();
									if (name.equals("paginginfo") && jsonReader.peek() != JsonToken.NULL) {
										pagingInfo = AppacitiveHelperMethods.readPagingInfo(jsonReader);
									} else if (name.equals("connections") && jsonReader.peek() != JsonToken.NULL) {
								...
								try {
									responseJsonObject = new JSONObject(buffer.toString());
									appacitiveError = AppacitiveHelperMethods.checkForErrorInStatus(responseJsonObject.getJSONObject("status"));
									if(appacitiveError == null) {
										readConnections(responseJsonObject.getJSONArray("connections"));
									}
								} catch (JSONException e) {
									e.printStackTrace();
								}
				...
				private void readConnections(JsonReader jsonReader) throws IOException {
	...
	public void searchForAllConnectionsWithRelationType(String relationType) {
		this.searchConnectionWithRelationTypeAndQueryString(relationType, null, null);
	}
	private void readConnection(JSONObject jsonObject) throws IOException,JSONException {
...
//		TODO : Uncomment the mRevision number and after the revision number is provided by the service in the response
		...
		try {
			...
			this.mUtcLastModifiedDate = fromJsonResponse(jsonObject.getString("__utclastupdateddate"));
		} catch (ParseException e) {
			e.printStackTrace();
		}
	}
	protected AppacitiveConnection readConnection(JsonReader jsonReader) throws IOException {
		jsonReader.beginObject();
		while (jsonReader.hasNext()) {
			if (jsonReader.peek() == JsonToken.NULL) {
				jsonReader.skipValue();
			...
			if (name.equals("__id") && jsonReader.peek() != JsonToken.NULL) {
				...
				this.mRelationType = jsonReader.toString();
	...
	private void readEndPointA(JsonReader jsonReader) throws IOException{
		...
		while (jsonReader.hasNext()) {
			...
			String name = jsonReader.nextName();
			if (jsonReader.equals("label") && jsonReader.peek() != JsonToken.NULL) {
				this.mLabelA = jsonReader.nextString();
			} else if (jsonReader.equals("articleid") && jsonReader.peek() != JsonToken.NULL) {
				this.mArticleAId = Long.parseLong(jsonReader.nextString());
			} else {
				jsonReader.skipValue();
			}
		}
	...
	private void readEndPointB(JsonReader jsonReader) throws IOException{
		...
		while (jsonReader.hasNext()) {
			...
			if (jsonReader.equals("label") && jsonReader.peek() != JsonToken.NULL) {
				...
				this.mArticleBId = Long.parseLong(jsonReader.nextString());
			} else {
				jsonReader.skipValue();
			}
		}
